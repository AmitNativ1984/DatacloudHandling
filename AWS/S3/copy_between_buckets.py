...
def main():
    ...
    parser.add_argument('--dest-prefix', type=str, default=None,
                        help='path inside bucket containg the new file')
    ...
    if not args.dest_prefix:
        args.dest_prefix = os.path.split(args.prefix)[0] + '_copy'
    ...
    pbar.set_description("copying to {}".format(args.dest_prefix))
    for key in selected_objects_keys:
        ...
        # copy_path_to_different_bucket(s3, objPath, destBucket=)

        copy_source = {"Bucket": args.source_bucket, "Key": key}
        dest_key = os.path.join(args.dest_prefix, os.path.split(key)[-1])
        success = s3.copy_object(CopySource=copy_source, Bucket=args.source_bucket, Key=dest_key)
